...
def pregunta_5(imgs, path):
    img = imgs[0, :, :]
    scale = 1.0
    w = int(img.shape[1] / scale)
    image = imutils.resize(img, width=w)
    ...
    for (xA, yA, xB, yB) in pick: cv2.rectangle(img, (xA, yA), (xB, yB), (0, 255, 0), 2)

    cv2.imshow("Output", orig)
    cv2.imshow("OutputNMS", img)
    cv2.waitKey(0)
    name1 = "Pregunta_5_{}.png".format(0)
    guardar(path, name1, orig)
    return None
